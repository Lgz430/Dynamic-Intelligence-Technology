...
/**
  * 函    数：OLED_ClearArea
  * 功    能：将OLED显存数组中指定区域的像素点清除（置0）
  * 参    数：X 指定区域左上角的横坐标，范围：-32768~32767，实际显示范围：0~127
  * 参    数：Y 指定区域左上角的纵坐标，范围：-32768~32767，实际显示范围：0~63
  * 参    数：Width 指定区域的宽度，范围：0~128
  * 参    数：Height 指定区域的高度，范围：0~64
  * 返 回 值：无
  * 说    明：调用此函数后要再次调用OLED_Update或OLED_UpdateArea来刷新显示
  */
void OLED_KillArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
    int16_t i, j;
    for (j = Y; j < Y + Height; j++)        // 遍历指定区域的每一行
    {
        for (i = X; i < X + Width; i++)     // 遍历指定区域的每一列
        {
            // 判断坐标是否在屏幕范围内
